...
router.get('/:slug', (req, res) => {
  ...
  const children = db.prepare('SELECT * FROM categories WHERE parent_id=? AND is_active=1 ORDER BY order_index').all(cat.id);
  const products = db.prepare('SELECT * FROM category_products WHERE category_id=? AND is_active=1 ORDER BY order_index').all(cat.id);
...
router.post('/', auth, (req, res) => {
  ...
  try {
    const result = db.prepare('INSERT INTO categories (name, slug, parent_id, description, image_url, order_index) VALUES (?,?,?,?,?,?)').run(name, slug, parent_id||null, description||'', image_url||null, order_index||0);
...
router.put('/:id', auth, (req, res) => {
  ...
  try {
    db.prepare('UPDATE categories SET name=?,slug=?,parent_id=?,description=?,image_url=?,order_index=?,is_active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(name, slug, parent_id||null, description||'', image_url||null, order_index||0, is_active===false?0:1, req.params.id);
...
router.get('/:id/products', auth, (req, res) => {
  const products = db.prepare('SELECT * FROM category_products WHERE category_id=? ORDER BY order_index').all(req.params.id);
  res.json(products);
});
...
router.post('/:id/products', auth, (req, res) => {
  ...
  const result = db.prepare('INSERT INTO category_products (category_id, brand, stock_code, stock_name, purpose, order_index) VALUES (?,?,?,?,?,?)').run(req.params.id, brand||'ORGAMİK', stock_code||'', stock_name, purpose||'', order_index||0);
...
router.put('/products/:pid', auth, (req, res) => {
  ...
  db.prepare('UPDATE category_products SET brand=?,stock_code=?,stock_name=?,purpose=?,order_index=?,is_active=? WHERE id=?').run(brand||'ORGAMİK', stock_code||'', stock_name, purpose||'', order_index||0, is_active===false?0:1, req.params.pid);
